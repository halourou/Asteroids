(function () {
  ...
  var Game = window.Asteroids.Game = function () {
    ...
    this.flash = this.flash || new window.Asteroids.Sprite({width: 462, height: 54,
      img: sub, numFrames: 7, ticksPerFrame: 1, game: this})
    this.shipHits = 0;
  };
  ...
  Game.prototype.draw = function(ctx) {
    ...
    this.allObjects().forEach(function (object) {
      if (object instanceof window.Asteroids.Ship && this.shipHits > 0) {
        this.overlayShip();
      } else {
    ...
    if (this.flash_pos !== null) {
      this.flash.render(ctx, this.ship.pos, shipAngle);
      this.flash.update();
    }
  ...
  Game.prototype.overlayShip = function () {
    this.flash_pos = this.ship.pos;
  }
