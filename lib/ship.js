(function() {
  ...
  Ship.prototype.power = function() {
    if (arguments[1]["key"] === 'up' && this.vel[1] >= -5) { this.vel[1] -= 1 };
    if (arguments[1]["key"] === 'down' && this.vel[1] <= 5) { this.vel[1] += 1 };
    if (arguments[1]["key"] === 'left' && this.vel[0] >= -5) { this.vel[0] -= 1 };
    if (arguments[1]["key"] === 'right' && this.vel[0] <= 5) { this.vel[0] += 1 };
  };
